    ...
    <div class="lg:pl-[320px]">
        <div class="max-w-[1200px] mx-auto p-4 md:p-8 lg:p-12">
            ...
            <div class="mb-12">
                ...
                <h1 class="text-h2 text-dark tracking-tighter">Edit Meal</h1>
                <p class="text-dark/40 font-bold text-xs uppercase tracking-[0.3em] mt-2">Update menu item details</p>
            </div>

            <!-- Form Card -->
            <div class="bg-white rounded-[2.5rem] p-8 md:p-12 border border-black/5 shadow-xl shadow-dark/5">
                <form action="{{ route('meal.update', $meal->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            <div class="space-y-2">
                                ...
                                <input type="text" name="mealName" id="mealName" required value="{{ old('mealName') ?? $meal->mealName }}"
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="mealType" id="mealType" required value="{{ old('mealType') ?? $meal->mealType }}"
                                    class="w-full bg-[#F8F8F8] border border-black/5 rounded-xl px-4 py-3 text-sm font-bold text-dark focus:ring-0 focus:border-dark transition-colors placeholder:text-dark/20">
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea name="mealIngredient" id="mealIngredient" rows="4" required
                                    class="w-full bg-[#F8F8F8] border border-black/5 rounded-xl px-4 py-3 text-sm font-bold text-dark focus:ring-0 focus:border-dark transition-colors placeholder:text-dark/20">{{ old('mealIngredient') ?? $meal->mealIngredient }}</textarea>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                @if($meal->mealImage)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $meal->mealImage) }}" class="w-20 h-20 rounded-lg object-cover border border-black/5">
                                </div>
                                @endif
                             ...
                             <div class="space-y-2">
                                ...
                                <textarea name="mealDescription" id="mealDescription" rows="4" required
                                    class="w-full bg-[#F8F8F8] border border-black/5 rounded-xl px-4 py-3 text-sm font-bold text-dark focus:ring-0 focus:border-dark transition-colors placeholder:text-dark/20">{{ old('mealDescription') ?? $meal->mealDescription }}</textarea>
                                @error('mealDescription') <p class="text-red-500 text-xs font-bold mt-1">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>

                    <div class="pt-8 border-t border-black/5 flex justify-end gap-4">
                         <a href="{{ route('meal.index') }}" class="px-8 py-4 bg-white border border-black/5 text-dark rounded-xl font-bold text-xs uppercase tracking-[0.2em] hover:bg-dark/5 transition-all">
                            Cancel
                        </a>
