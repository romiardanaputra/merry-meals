{{-- Member Meal Package Selection View --}}
...
<main class="min-h-screen bg-[#F8F8F8] font-inter p-4 sm:p-8 lg:p-12 overflow-x-hidden">
    <div class="max-w-[1200px] mx-auto">
        ...
        {{-- Header --}}
        <div class="mb-12">
            <h1 class="text-h2 text-dark tracking-tighter">Select Your Package</h1>
            <p class="text-dark/40 font-bold text-xs uppercase tracking-[0.3em] mt-2 italic">Choose a delivery frequency for {{ $meal->mealName }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <form action="{{ route('member.meals.order') }}" method="POST" class="h-full">
                ...
                <button type="submit" class="w-full h-full text-left group">
                    <div class="bg-white rounded-[2rem] p-8 border-2 {{ isset($package['popular']) ? 'border-primary shadow-xl shadow-primary/10' : 'border-black/5' }} hover:border-primary hover:shadow-xl hover:shadow-primary/10 transition-all duration-500 flex flex-col h-full relative overflow-hidden">
                        @if(isset($package['popular']))
                            <div class="absolute top-0 right-0 bg-primary text-dark px-4 py-1 rounded-bl-xl text-[9px] font-black uppercase tracking-widest">Popular</div>
                        @endif
                        
                        <div class="w-16 h-16 bg-dark/5 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-primary/10 transition-colors">
                            <svg class="w-8 h-8 text-dark/40 group-hover:text-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $package['icon'] }}" />
                            </svg>
                        </div>
                        
                        <h3 class="text-xl font-black text-dark tracking-tight mb-2">{{ $package['name'] }}</h3>
                        <p class="text-[10px] font-black uppercase tracking-widest text-primary mb-4">{{ $package['duration'] }}</p>
                        <p class="text-sm text-dark/60 font-medium leading-relaxed flex-1">{{ $package['description'] }}</p>
                        
                        <div class="mt-8 pt-6 border-t border-black/5">
                            <span class="inline-flex items-center text-[10px] font-black uppercase tracking-widest text-dark/30 group-hover:text-primary transition-colors">
                                Select Package
